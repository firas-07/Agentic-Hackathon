...
async def chat_with_knowledge_base(request: QueryRequest):
    ...
    try:
        ...
        answer, thread_id = agent.generate_answer(request.query, matches, request.thread_id)
        
        # Generate conversation title if this is a new conversation
        conversation_title = None
        if thread_id in agent.conversations:
            if agent.conversations[thread_id]["title"] is None:
                ...
                conversation_title = agent.conversations[thread_id]["title"]
